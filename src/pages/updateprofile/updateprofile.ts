      ...
      this.firebaseDB.database
        ...
        .then(
          () => {
            ...
            loading
              .present()
              .then(_ => this.navCtrl.pop().then(_ => loading.dismiss()))
              .then(_ => {
                this.firebaseDB.database
                  .ref(`userProfile/${user.uid}`)
                  .once("value", data => {
                    this.event.publish("user:loggedIn", data.val());
                  });
              });
          },
          err => {
  ...
  camerafunction(options) {
    this.camera.getPicture(options).then(
      imgdata => {
        ...
        pictures
          .child(`profilepic/avatar-${this.fire.auth.currentUser.uid}.jpg`)
          .putString(imgdata, "base64", { contentType: "image/jpeg" })
          .then(
            savedProfilePicture => {
              this.fire.auth.onAuthStateChanged(auth => {
                this.firebaseDB.database
                  .ref(`/userProfile/${auth.uid}`)
                  .update({
