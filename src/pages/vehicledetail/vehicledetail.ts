...
export class VehicledetailPage {
  ...
  ionViewDidLoad() {
    ...
    this.fire.authState.subscribe(user => {
      this.firebaseDB
        .object(`vehicle/${user.uid}/${this.car.plate}`)
        .subscribe(result => {
          console.log(result);
          this.car = result;
        });
        ...
        },
        {
          ...
          handler: () => {
            this.firebaseDB.database
              .ref(
                `/vehicle/${this.fire.auth.currentUser.uid}/${this.car.plate}`
              )
              .update({
                type: this.car.type,
  ...
  camerafunction(options) {
    this.camera.getPicture(options).then(
      imgdata => {
        ...
        pictures
          .child(`vehiclePic/${this.car.plate}/${this.car.plate}.jpg`)
          .putString(imgdata, "base64", { contentType: "image/jpeg" })
          .then(
            savedPVehiclePicture => {
  ...
  deleteAvatar() {
    ...
    const pictures = firebase
      .storage()
      .ref(
        `${this.fire.auth.currentUser.uid}/vehiclePic/${this.car.plate}/${this
          .car.plate}.jpg`
      );
